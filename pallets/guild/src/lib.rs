...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_guild(
			...
			let guild = Guild { owner: sender.clone(), members: Vec::new(), minimum_balance };
		...
		pub fn callback(origin: OriginFor<T>, result: Vec<u8>) -> DispatchResult {
			...
			ensure!(<Guilds<T>>::contains_key(request.guild_id), Error::<T>::GuildDoesNotExist);
			// Unwrap is fine here because we check its existence previously
		...
		pub fn join_guild(
			...
			ensure!(<Guilds<T>>::contains_key(guild_id), Error::<T>::GuildDoesNotExist);
			...
			JoinRequests::<T>::insert(
				request_id,
				JoinRequest::<T::AccountId> { requester: sender.clone(), guild_id },
			);
